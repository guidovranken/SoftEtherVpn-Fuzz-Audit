...
#ifdef FUZZING
/* This is a "minimal" version of NewIPC to be used on fuzzing mode where a process depends on
 * creating an IPC, but is not interested in going through the regular process
 */
IPC *FuzzingNewIPC(CEDAR *cedar, char *client_name, char *postfix, char *hubname, char *username, char *password,
			...
			bool bridge_mode, UINT mss, EAP_CLIENT *eap_client)
{
	...
	FreePack(p);
    */
    memset(ipc->random, 0xBB, sizeof(ipc->random));
    StrCpy(server_str, sizeof(ipc->SessionName), "X");
    server_ver = 0x1234;
    ...
    memset(unique, 0xAA, sizeof(unique));
    ...
    */
    StrCpy(ipc->SessionName, sizeof(ipc->SessionName), "X");
    StrCpy(ipc->ConnectionName, sizeof(ipc->ConnectionName), "Y");
	ipc->Policy = ZeroMalloc(sizeof(POLICY));
    Recv((SOCK*)8, ipc->Policy, sizeof(POLICY), false);
    //if ( Recv((SOCK*)8, ipc->Policy, sizeof(POLICY), false) != sizeof(POLICY) ) {
    //    goto LABEL_ERROR;
    //}

    Recv((SOCK*)8, ipc->MacAddress, 6, false);
	/*if (PackGetData2(p, "IpcMacAddress", ipc->MacAddress, 6) == false || IsZero(ipc->MacAddress, 6))*/
	if (IsZero(ipc->MacAddress, 6))
	{
		err = ERR_PROTOCOL_ERROR;
		/*FreePack(p);*/
		goto LABEL_ERROR;
    ...
    */
    Recv((SOCK*)8, mschap_v2_server_response_20, sizeof(mschap_v2_server_response_20), false);
    Copy(ipc->MsChapV2_ServerResponse, mschap_v2_server_response_20, sizeof(mschap_v2_server_response_20));
...
#else
				if ((Endian32(dhcp->Header->TransactionId) & 0xF) == (tran_id & 0xF) && dhcp->OpCode == expecting_code)
#endif
				{
...
#else
	TubeSendEx((TUBE*)8, data, size, NULL, true);
#endif
}
...
BLOCK *IPCRecvL2(IPC *ipc)
{
    ...
    void* data;
    UINT ret = Recv((SOCK*)8, recvbuf, sizeof(recvbuf), false);
    if ( ret == 0 || ret == SOCK_LATER ) {
        return NULL;
    }
    data = Malloc(ret);
    memcpy(data, recvbuf, ret);
	b = NewBlock(data, ret, 0);
#endif
